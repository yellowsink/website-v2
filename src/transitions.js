			...
			link.addEventListener("click", (e) => {
				...
				fetch(link.href).then((r) => {
					document.createDocumentTransition().start(async () => {
						const newDom = new DOMParser().parseFromString(await r.text(), "text/html");

						history.replaceState(null, "", link.href);

						document.getElementById("content").innerHTML = newDom.getElementById("content").innerHTML;

						document.querySelector("#sidebar ul").innerHTML = newDom.querySelector("#sidebar ul").innerHTML;
